...
```
To retrieve the complete underlying data use the `samples()` function:

```{r downloadData}
Amarete_site <- samples(mySites[[1]])
```
The `samples()` function is under active development and may return an error for some sites.
